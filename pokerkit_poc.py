    ...
    def reduce(
        ...
        for action in legal_actions:
            ...
            if name in {"bet_or_raise", "bet", "raise", "complete_bet_or_raise_to"}:
                ...
                allowed_exact_amounts = set(street_rule.opening_raise_amounts)
                if not allowed_exact_amounts:
                    allowed_exact_amounts = set(street_rule.bet_amounts)

                pot_targets: Tuple[int, ...] = ()
                if street_rule.allowed_bet_pcts:
                    pot_targets = self._nearest_pot_targets(state, street_rule.allowed_bet_pcts)
                    allowed_exact_amounts |= set(pot_targets)

                # Preserve the canonical PokerKit legal family and apply the policy as a
                # state-aware filter, not as a hard remapping to a single exact amount set.
                # For pot-percentage rules we use the targets as soft hints rather than as a
                # requirement that every legal bet value match exactly; PokerKit already
                # generated the valid family for this state.
                ...
                if street_rule.allowed_bet_pcts and not pot_targets:
                    pass
                elif allowed_exact_amounts and amount_int not in allowed_exact_amounts:
                    continue
